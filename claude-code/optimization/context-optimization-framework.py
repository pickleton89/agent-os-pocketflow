...
class ContextOptimizer:
    ...
    def analyze_context_usage(self, context_data: Dict[str, Any],
        ...
        for field_name, field_value in context_data.items():
            ...
            usage_analysis['field_usage'][field_name] = {
                ...
                'usage_ratio': len(required_by) / len(target_agents) if target_agents else 0
            }
        ...
        for field_name, usage in usage_analysis['field_usage'].items():
            if usage['usage_ratio'] < 0.5 and usage['token_cost'] > 100:
                usage_analysis['optimization_opportunities'].append({
                    ...
                    'potential_savings': usage['token_cost'] * (1 - usage['usage_ratio']),
    ...
    def optimize_context_for_agent(self, context_data: Dict[str, Any],
        ...
        for field_name in req.required_fields:
            if field_name in context_data:
                ...
                print(f"⚠️  Warning: Required field '{field_name}' missing for {agent_name}")

        # Include optional fields based on priority and token budget
        current_tokens = sum(self._estimate_token_cost(v) for v in optimized_context.values())
        ...
        for field_name in req.optional_fields:
            if field_name in context_data:
                ...
                priority = self.field_priorities.get(field_name, 3)
                efficiency = 1 / (priority * token_cost)  # Higher efficiency = better value
        ...
        for field_name, priority, token_cost, efficiency in optional_fields:
            ...
            elif priority <= 2:  # Critical/important fields get compressed instead of excluded
        ...
        optimized_context['_agent_context'] = {
            ...
            'excluded_fields': [f for f in context_data.keys() if f not in optimized_context and not f.startswith('_')]
        }

        return optimized_context

    def create_parallel_contexts(self, context_data: Dict[str, Any],
                               agent_names: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    def _extract_shared_context(self, context_data: Dict[str, Any],
    ...
    def _estimate_token_cost(self, value: Any) -> int:
        ...
        if isinstance(value, str):
            ...
            return sum(self._estimate_token_cost(k) + self._estimate_token_cost(v)
    ...
    def _compress_field_value(self, value: Any) -> Any:
        ...
        if isinstance(value, str):
            ...
            for key in value.keys():
                if any(keyword in key.lower() for keyword in ['key', 'main', 'primary', 'core', 'important']):
    ...
    def generate_optimization_report(self, context_data: Dict[str, Any],
        ...
        report.append(f"- **Average per Agent**: {optimized_total // len(agent_names):,} tokens")
        report.append(f"- **Memory Efficiency**: {(original_size * len(agent_names) - optimized_total) / (original_size * len(agent_names)):.1%} reduction")
        report.append("")
        ...
        for field_name, usage in analysis['field_usage'].items():
            priority_name = {1: "Critical", 2: "Important", 3: "Optional"}[usage['priority']]
            required_by = ", ".join(usage['required_by'][:3])  # Limit display
            if len(usage['required_by']) > 3:
                required_by += f" (+{len(usage['required_by']) - 3} more)"

            report.append(f"| {field_name} | {priority_name} | {usage['token_cost']} | {usage['usage_ratio']:.1%} | {required_by} |")
        ...
        if analysis['optimization_opportunities']:
            ...
            for i, opp in enumerate(analysis['optimization_opportunities'], 1):
                ...
                report.append(f"**Potential Savings**: {opp['potential_savings']:.0f} tokens")
        ...
        for agent_name, ctx in optimized_contexts.items():
            ...
            agent_optimized = sum(self._estimate_token_cost(v) for v in ctx.values() if not str(v).startswith('_'))
            reduction = (agent_original - agent_optimized) / agent_original if agent_original > 0 else 0
            excluded = len(ctx.get('_agent_context', {}).get('excluded_fields', []))

            report.append(f"| {agent_name} | {agent_original} | {agent_optimized} | {reduction:.1%} | {excluded} fields |")
        ...
        high_cost_fields = [f for f, u in analysis['field_usage'].items()
        ...
        if high_cost_fields:
            ...
            for field in high_cost_fields:
                report.append(f"- **{field}**: {analysis['field_usage'][field]['token_cost']} tokens")
            report.append("")

        underused_fields = [f for f, u in analysis['field_usage'].items()
                          if u['usage_ratio'] < 0.3 and u['token_cost'] > 200]
        if underused_fields:
            report.append("### Underused Fields")
            report.append("These fields have high token cost but low usage - consider agent-specific inclusion:")
            for field in underused_fields:
                usage = analysis['field_usage'][field]
                report.append(f"- **{field}**: {usage['token_cost']} tokens, used by {usage['usage_ratio']:.1%} of agents")
            report.append("")

        report.append("### General Recommendations")
        report.append("1. **Use agent-specific contexts** for parallel execution to minimize token usage")
        ...
        report.append("4. **Monitor field usage patterns** and adjust priorities based on actual needs")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Context optimization for document creation agents")
    ...
    parser.add_argument("--agents", nargs="+", help="Target agent names",
                       default=["mission-document-creator", "tech-stack-document-creator", "roadmap-document-creator"])
    ...
    if args.analyze:
        ...
        context_data = {
            ...
            "target_users": [
                {"role": "Small business owner", "age": "25-45", "context": "Limited time for content creation"},
                {"role": "Marketing manager", "age": "28-40", "context": "Needs efficient content workflows"}
            ],
            "tech_stack": {
        ...
        optimized_contexts = optimizer.create_parallel_contexts(context_data, args.agents)

        output_file = Path(args.output) if args.output else context_file.with_suffix('.optimized.json')
        ...
        # Show summary
        original_size = sum(optimizer._estimate_token_cost(v) for v in context_data.values())
        total_optimized = sum(
