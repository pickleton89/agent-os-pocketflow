...
- Many workflows assume macOS tooling (e.g., `afplay` in post‑execution tasks). Broader OS support may be added later.
...
- Prefer project‑local standards: edit `.agent-os/standards/` inside your repo.
- Base standards at `~/.agent-os/standards/` apply across projects and act as defaults.
- Instruction lookups prefer project‑local paths in `.agent-os/instructions/` and may fall back to base (`~/.agent-os/instructions/`) when needed.

#### Instruction Resolution Order
When an instruction is referenced, resolution occurs in this order:
1) `.agent-os/instructions/core`
2) `.agent-os/instructions`
3) `~/.agent-os/instructions/core`
4) `~/.agent-os/instructions`

This ensures project‑local overrides take precedence over the base installation.
...
# .agent-os/instructions/
# .agent-os/standards/
# .agent-os/checklists/ (after running /plan-product)
