...
class DocsCompatibilityChecker:
    ...
    def _check_framework_version(self):
        ...
        if self._compare_versions(framework_version, current_version) < 0:
            self.warnings.append(
                f"Registry created with older framework version {framework_version}, current is {current_version}"
            )
        elif self._compare_versions(framework_version, current_version) > 0:
            self.warnings.append(
                f"Registry created with newer framework version {framework_version}, current is {current_version}"
            )
        else:
            self.info.append(
                f"Framework version {framework_version} matches current version"
            )
    ...
    def _check_tech_stack_versions(self):
        ...
        for name, config in tech_stack.items():
            ...
            if version:
                ...
                self.info.append(
                    f"No version specified for {name} - consider adding for better compatibility tracking"
                )
    ...
    def _check_specific_tech_version(self, name: str, version: str, tech_type: str):
        ...
        known_compatibility = {
            "react": {"deprecated_below": "16.8.0", "recommended_minimum": "18.0.0"},
            "fastapi": {"deprecated_below": "0.68.0", "recommended_minimum": "0.100.0"},
            "python": {"deprecated_below": "3.8.0", "recommended_minimum": "3.10.0"},
        }
        ...
        if name.lower() in known_compatibility:
            ...
            if "deprecated_below" in compat:
                if self._compare_versions(version, compat["deprecated_below"]) < 0:
                    self.warnings.append(
                        f"{name} version {version} is deprecated (minimum supported: {compat['deprecated_below']})"
                    )

            if "recommended_minimum" in compat:
                if self._compare_versions(version, compat["recommended_minimum"]) < 0:
                    self.info.append(
                        f"{name} version {version} is below recommended minimum {compat['recommended_minimum']}"
                    )
    ...
    def _check_url_accessibility(self, name: str, url: str, section: str):
        ...
        try:
            ...
            if status_code == 200:
                ...
                self.warnings.append(
                    f"{section}.{name}: URL returned {status_code}: {url}"
                )
            else:
                self.info.append(
                    f"{section}.{name}: URL accessible with status {status_code}: {url}"
                )
            ...
            if e.code == 404:
                ...
                self.info.append(
                    f"{section}.{name}: URL accessible with status {e.code}: {url}"
                )
        except URLError as e:
            if "timed out" in str(e).lower():
                self.warnings.append(
                    f"{section}.{name}: URL timeout (may be slow): {url}"
                )
            else:
                self.errors.append(
                    f"{section}.{name}: Cannot connect to URL: {url} ({e})"
                )
        except Exception as e:
            self.warnings.append(
                f"{section}.{name}: Unexpected error checking URL: {url} ({e})"
            )
    ...
    def _check_local_path(self, name: str, path: str, section: str):
        ...
        if full_path.exists():
            if full_path.is_file():
                ...
                self.info.append(
                    f"{section}.{name}: Local directory accessible ({path})"
                )
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check documentation registry compatibility"
    )
