...
class DocsRegistryValidator:
    ...
    REQUIRED_METADATA = {"created_by", "framework_version", "registry_format_version"}
    ...
    def _validate_tech_stack_entry(self, name: str, config: Dict[str, Any]):
        ...
        if missing_fields:
            self.errors.append(
                f"Tech stack '{name}' missing required fields: {missing_fields}"
            )

        # Validate type
        if config.get("type") not in self.VALID_TECH_STACK_TYPES:
            self.warnings.append(
                f"Tech stack '{name}' has unexpected type: {config.get('type')}"
            )
    ...
    def _validate_external_api_entry(self, name: str, config: Dict[str, Any]):
        ...
        if missing_fields:
            self.errors.append(
                f"External API '{name}' missing required fields: {missing_fields}"
            )

        # Validate type
        if config.get("type") not in self.VALID_API_TYPES:
            self.warnings.append(
                f"External API '{name}' has unexpected type: {config.get('type')}"
            )
    ...
    def _validate_internal_standard_entry(self, name: str, config: Dict[str, Any]):
        ...
        if missing_fields:
            self.errors.append(
                f"Internal standard '{name}' missing required fields: {missing_fields}"
            )

        # Validate type
        if config.get("type") not in self.VALID_STANDARD_TYPES:
            self.warnings.append(
                f"Internal standard '{name}' has unexpected type: {config.get('type')}"
            )
    ...
    def _validate_compliance_entry(self, name: str, config: Dict[str, Any]):
        ...
        if missing_fields:
            self.errors.append(
                f"Compliance '{name}' missing required fields: {missing_fields}"
            )

        # Validate type
        if config.get("type") not in self.VALID_COMPLIANCE_TYPES:
            self.warnings.append(
                f"Compliance '{name}' has unexpected type: {config.get('type')}"
            )
    ...
    def _validate_pocketflow_pattern_entry(self, name: str, config: Dict[str, Any]):
        ...
        if not isinstance(config, dict):
            self.errors.append(
                f"PocketFlow pattern entry '{name}' must be a dictionary"
            )
            return
        ...
        if missing_fields:
            self.errors.append(
                f"PocketFlow pattern '{name}' missing required fields: {missing_fields}"
            )

        # Validate type (similar to internal standards)
        if config.get("type") not in self.VALID_STANDARD_TYPES:
            self.warnings.append(
                f"PocketFlow pattern '{name}' has unexpected type: {config.get('type')}"
            )
    ...
    def _validate_discovery_settings(self):
        ...
        # Check required fields
        missing_fields = self.REQUIRED_DISCOVERY_SETTINGS - set(
            discovery_settings.keys()
        )
        if missing_fields:
            self.errors.append(
                f"discovery_settings missing required fields: {missing_fields}"
            )
        ...
        for path_field in ["scan_paths", "include_patterns", "exclude_patterns"]:
            if path_field in discovery_settings:
                if not isinstance(discovery_settings[path_field], list):
                    self.errors.append(
                        f"discovery_settings.{path_field} must be a list"
                    )
    ...
    def _check_staleness(self):
        ...
        try:
            ...
            if days_since_update > 90:
                self.warnings.append(
                    f"Registry hasn't been updated in {days_since_update} days - consider reviewing"
                )
            elif days_since_update > 30:
                self.warnings.append(
                    f"Registry is {days_since_update} days old - consider reviewing"
                )
        except ValueError:
