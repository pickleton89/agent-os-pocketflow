...
class NodeLifecycleAnalyzer(ast.NodeVisitor):
    ...
    def _validate_node_lifecycle(self, node_class: Dict[str, Any]):
        ...
        is_async = any(
            base
            in ["AsyncNode", "BatchNode", "AsyncBatchNode", "AsyncParallelBatchNode"]
            for base in bases
        )
    ...
    def _ast_to_string(self, node: ast.AST) -> str:
        ...
        if hasattr(ast, "unparse"):
            ...
            if isinstance(node, ast.Attribute):
                value_str = (
                    self._ast_to_string(node.value)
...
class BestPracticesValidator:
    ...
    def _has_collection_processing_pattern(self, content: str) -> bool:
        ...
        return iteration_count >= 2 or any(
            re.search(pattern, content) for pattern in patterns[:3]
        )
    ...
    def _has_complex_logic_pattern(self, content: str) -> bool:
        ...
        return any(
            re.search(pattern, content, re.DOTALL) for pattern in complex_patterns
        )

    def validate_directory(
        self, directory: Path, checks: Optional[List[str]] = None
    ) -> List[BestPracticeViolation]:
...
def format_violations(violations: List[BestPracticeViolation]) -> str:
    ...
    for level_name, level_violations in [
        ("ERRORS", errors),
        ("WARNINGS", warnings),
        ("INFO", infos),
    ]:
        if level_violations:
            ...
            for violation in level_violations:
                ...
                location = (
                    f"{file_name}:{violation.line_number}"
...
Examples:
  ...
  python validate-best-practices.py --checks lifecycle,batch
    ...
    parser.add_argument(
        "--workflow", type=Path, help="Specific workflow directory to validate"
    )
    ...
    if args.workflow:
        ...
        target_dir = (
            ...
            if Path(".agent-os/workflows").exists()
            else Path(".")
        )
