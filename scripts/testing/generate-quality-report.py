...
class QualityReportGenerator:
    ...
    def collect_phase4_test_results(self) -> Dict[str, Any]:
        ...
        phase4_script = (
            ...
            / "claude-code"
            / "testing"
            / "test-phase4-optimization.py"
        )

        if not phase4_script.exists():
            return {"available": False, "error": "Phase 4 test script not found"}
    ...
    def collect_performance_benchmarks(self) -> Dict[str, Any]:
        ...
        with tempfile.NamedTemporaryFile(
            ...
            if result["success"] and Path(tmp.name).exists():
                ...
                benchmark_results = {
                    "error": result.get("error", "Benchmark execution failed")
                }
    ...
    def collect_validation_test_results(self) -> Dict[str, Any]:
        ...
        if not validation_script.exists():
            return {"available": False, "error": "Validation test script not found"}

        result = self.run_command_safely(["bash", str(validation_script), "--quick"])
    ...
    def collect_end_user_workflow_results(self) -> Dict[str, Any]:
        ...
        workflow_script = (
            ...
            / "scripts"
            / "validation"
            / "validate-end-user-workflows.sh"
        )
        ...
        result = self.run_command_safely(["bash", str(workflow_script)])
        ...
        if result["success"] or result["returncode"] == 1:
            ...
            for line in output_lines:
                if "Total Tests Run:" in line:
                    ...
                    "FAILED" in line or "failed" in line
                ):
    ...
    def generate_report_data(self) -> Dict[str, Any]:
        ...
        report_data["overall_summary"] = self._calculate_overall_summary(
            report_data["test_components"]
        )
    ...
    def _calculate_overall_summary(
        ...
        for component_name, component_data in test_components.items():
            if component_data.get("available", False):
                ...
                if component_name == "phase4_optimization":
                    ...
                    overall_success = benchmark_results.get("summary", {}).get(
                    ...
                    component_success = (
                        suites_run > 0 and (suites_passed / suites_run) >= 0.8
                    )
                    ...
                    component_success = (
                        tests_run > 0 and (tests_passed / tests_run) >= 0.8
                    )
        ...
        if summary["components_available"] > 0:
            summary["health_score"] = (
                summary["components_passed"] / summary["components_available"]
            ) * 100
        ...
        if summary["components_available"] < summary["total_components"]:
            summary["recommendations"].append(
                "Ensure all test components are properly installed and accessible"
            )
...
"""
    ...
    def save_reports(
        self, report_data: Dict[str, Any], formats: List[str] = ["html", "json"]
    ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate comprehensive quality assurance report"
    )
    parser.add_argument(
    ...
    print(
        f"Components: {summary['components_passed']}/{summary['components_available']} passed"
    )
