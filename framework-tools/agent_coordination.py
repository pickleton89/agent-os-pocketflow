...
class PatternOverrideManager:
    ...
    def apply_user_overrides(
        self, recommendation: PatternRecommendation, user_overrides: Dict[str, Any]
        ...
        if "force_pattern" in user_overrides:
            ...
            )  # Reduce confidence for forced patterns
            recommendation.rationale += (
                f" [USER OVERRIDE: Pattern forced to {forced_pattern.value}]"
            )
        ...
        if "complexity_preference" in user_overrides:
            ...
            estimated_nodes = recommendation.workflow_suggestions.get(
                "estimated_nodes", 4
            )
    ...
    def validate_pattern_combination(
        self, primary: PatternType, secondary: List[PatternType]
    ) -> bool:
        ...
        if primary in conflicts:
            ...
            for secondary_pattern in secondary:
                if secondary_pattern in conflicting_patterns:
                    logger.warning(
                        f"Pattern conflict detected: {primary.value} + {secondary_pattern.value}"
                    )
                    return False
...
class AgentCoordinator:
    ...
    def create_handoff_to_subagent(
        self, context: CoordinationContext, target_agent: str = None
    ) -> HandoffPackage:
        ...
        if not context.pattern_recommendation:
            raise ValueError(
                "Pattern recommendation required for PocketFlow agent handoff"
            )
        ...
        payload = {
            ...
            "pattern_analysis": {
                ...
                "secondary_patterns": [
                    p.value for p in context.pattern_recommendation.secondary_patterns
                ],
                "rationale": context.pattern_recommendation.rationale,
                "detailed_justification": getattr(
                    context.pattern_recommendation, "detailed_justification", ""
                ),
        ...
        self._log_coordination(
            ...
            payload["coordination_metadata"]["coordination_id"],
        )
    ...
    def process_subagent_feedback(
        self, handoff: HandoffPackage, subagent_response: Dict[str, Any]
    ) -> CoordinationContext:
        ...
        context.orchestrator_feedback = (
            subagent_response  # Keep field name for compatibility
        )
        ...
        if "pattern_modifications" in subagent_response:
            ...
            logger.info(
                f"{target_agent} suggested pattern modifications: {modifications}"
            )
            ...
            if "complexity_adjustment" in modifications:
                ...
                context.pattern_recommendation.template_customizations["complexity"] = (
                    new_complexity
                )

            if "additional_nodes" in modifications:
                additional_nodes = modifications["additional_nodes"]
                context.pattern_recommendation.workflow_suggestions[
                    "additional_nodes"
                ] = additional_nodes

        # Update coordination history
        context.coordination_history.append(
            f"{target_agent} feedback processed: {subagent_response.get('status', 'unknown')}"
        )
        ...
        self._log_coordination(
            "feedback_processed",
            f"status_{subagent_response.get('status')}",
            context.coordination_history[-1],
        )

        return context

    def handle_pattern_override_request(
        self, context: CoordinationContext, override_request: Dict[str, Any]
    ) -> CoordinationContext:
        ...
        if "pattern" in override_request:
            try:
                ...
                logger.info(
                    f"User requested pattern override to: {requested_pattern.value}"
                )
            except ValueError:
                logger.error(
                    f"Invalid pattern requested: {override_request['pattern']}"
                )
                return context
        ...
        if context.pattern_recommendation:
            context.pattern_recommendation = self.override_manager.apply_user_overrides(
                context.pattern_recommendation, override_request
            )

        # Update coordination history
        context.coordination_history.append(
            f"Pattern override applied: {override_request}"
        )
        ...
        self._log_coordination(
            "override_applied", "pattern_override", str(override_request)
        )
    ...
    def create_coordination_summary(
        ...
        summary = {
            ...
            "pattern_selected": context.pattern_recommendation.primary_pattern.value
            if context.pattern_recommendation
            else "none",
            "confidence_score": context.pattern_recommendation.confidence_score
            if context.pattern_recommendation
            ...
            "final_customizations": context.pattern_recommendation.template_customizations
            if context.pattern_recommendation
            else {},
        ...
        self._log_coordination(
            "summary_generated", "coordination_complete", summary["pattern_selected"]
        )
...
def coordinate_pattern_analysis(
    project_name: str,
    requirements: str,
    user_overrides: Optional[Dict[str, Any]] = None,
) -> CoordinationContext:
    ...
    if user_overrides:
        recommendation = coordinator.override_manager.apply_user_overrides(
            recommendation, user_overrides
        )
    ...
    logger.info(
        f"Pattern analysis completed: {recommendation.primary_pattern.value} (confidence: {recommendation.confidence_score:.2f})"
    )

    return context


def create_subagent_handoff(
    context: CoordinationContext, target_agent: str = None
) -> HandoffPackage:
...
if __name__ == "__main__":
    ...
    context = coordinate_pattern_analysis(
        project_name="SmartDocProcessor", requirements=test_requirements
    )

    print(
        f"Pattern Analysis Result: {context.pattern_recommendation.primary_pattern.value}"
    )
    ...
    print(
        f"Design handoff: {design_handoff.source_agent} -> {design_handoff.target_agent}"
    )

    # Test pattern override
    coordinator = AgentCoordinator()
    override_request = {"force_pattern": "TOOL", "complexity_preference": "simple"}
    updated_context = coordinator.handle_pattern_override_request(
        context, override_request
    )
    print(
        f"After override: {updated_context.pattern_recommendation.primary_pattern.value}"
    )

    # Generate final summary
    summary = coordinator.create_coordination_summary(updated_context)
    print(
        f"Coordination Summary: {summary['success']} - {summary['coordination_steps']} steps"
    )
