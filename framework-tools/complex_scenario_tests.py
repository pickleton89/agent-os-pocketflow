...
class ComplexScenarioTester:
    ...
    def test_hybrid_requirements(self):
        ...
        print(
            f"Secondary Patterns: {[p.value for p in recommendation.secondary_patterns]}"
        )

        # Should identify RAG as primary with TOOL and AGENT as secondary
        assert recommendation.primary_pattern.value in ["RAG", "AGENT"], (
            "Should identify RAG or AGENT for hybrid system"
        )
        assert len(recommendation.secondary_patterns) >= 1, (
    ...
    def test_ambiguous_requirements(self):
        ...
        assert recommendation.confidence_score < 0.4, (
            "Ambiguous requirements should have low confidence"
        )
        assert recommendation.primary_pattern.value == "WORKFLOW", (
            "Should default to WORKFLOW for ambiguous requirements"
        )
    ...
    def test_enterprise_complexity(self):
        ...
        print(f"Confidence: {recommendation.confidence_score:.2f}")
        print(
            f"Template Customizations: {list(recommendation.template_customizations.keys())}"
        )

        # Should identify high complexity and suggest appropriate patterns
        assert recommendation.confidence_score > 0.5, (
            "Clear enterprise requirements should have high confidence"
        )
        assert "enterprise" in recommendation.detailed_justification.lower(), (
            "Should identify enterprise complexity"
        )
    ...
    def test_multi_domain_integration(self):
        ...
        assert recommendation.primary_pattern.value in [
            "MULTI-AGENT",
            "WORKFLOW",
            "AGENT",
    ...
    def test_security_focused_requirements(self):
        ...
        assert any(
            keyword in str(recommendation.template_customizations).lower()
            for keyword in security_keywords
        ), "Should identify security requirements"
    ...
    def test_real_time_systems(self):
        ...
        assert (
            "real" in str(recommendation.template_customizations).lower()
            or "stream" in str(recommendation.template_customizations).lower()
    ...
    def test_legacy_system_integration(self):
        ...
        assert len(recommendation.template_customizations) > 0, (
            "Should suggest integration-specific customizations"
        )
...
def test_edge_cases():
    ...
    try:
        ...
        print(
            f"Empty requirements: {result.primary_pattern.value} (confidence: {result.confidence_score:.2f})"
        )
    except Exception as e:
    ...
    print(
        f"Long requirements: {result.primary_pattern.value} (confidence: {result.confidence_score:.2f})"
    )
    ...
    print(
        f"Tech jargon: {result.primary_pattern.value} (confidence: {result.confidence_score:.2f})"
    )
