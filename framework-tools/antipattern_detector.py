...
class PocketFlowASTVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        elif self.current_class and node.name in [
            "prep",
            "exec",
            "exec_async",
            "post",
    ...
    def _check_monolithic_node(
        self, class_node: ast.ClassDef, methods: Dict[str, Any]
    ) -> None:
    ...
    def _check_node_lifecycle_violations(
        self, class_node: ast.ClassDef, methods: Dict[str, Any]
    ) -> None:
            ...
            self.violations.append(
                AntipatternViolation(
                    ...
                    context={
                        ...
                        "branches": if_else_count,
                    },
                )
            )

    def _check_lifecycle_confusion(
        self, class_node: ast.ClassDef, methods: Dict[str, Any]
    ) -> None:
    ...
    def _count_method_lines(self, method_node: ast.FunctionDef) -> int:
        ...
        if end_line == start_line:
            ...
            estimated_lines = (
                1 + statement_count + max(0, statement_count // 3)
            )  # Add ~33% for whitespace
    ...
    def _is_pocketflow_node_class(self, base_names: List[str]) -> bool:
        ...
        framework_patterns = ["MockNode", "TestNode", "BaseNode", "FrameworkNode"]
...
class AntipatternDetector:
    ...
    def detect_file(self, file_path: str) -> List[AntipatternViolation]:
        ...
        try:
            ...
            regex_violations = self._detect_regex_patterns(
                normalized_path, source_code, visitor.is_test_file
            )
    ...
    def _adjust_severity_for_test_context(
        self, severity: Severity, is_test_file: bool
    ) -> Severity:
    ...
    def _detect_regex_patterns(
        self, file_path: str, source_code: str, is_test_file: bool = False
        ...
        for i, line in enumerate(lines, 1):
            if re.search(r"for\s+\w+\s+in\s+.*:", line):
                ...
                if self._is_in_exec_method(lines, i - 1) and "BatchNode" not in "".join(
                    lines[max(0, i - 10) : i + 10]
                ):
                    violations.append(
                        AntipatternViolation(
                            antipattern_id="sync_collection_processing",
                            name="Synchronous Collection Processing",
                            severity=self._adjust_severity_for_test_context(
                                Severity.MEDIUM, is_test_file
                            ),
                            file_path=file_path,
                            line_number=i,
        ...
        for i, line in enumerate(lines, 1):
            for pattern in blocking_io_patterns:
                if re.search(pattern, line):
                    if self._is_in_exec_method(
                        lines, i - 1
                    ) and "AsyncNode" not in "".join(lines[max(0, i - 10) : i + 10]):
                        violations.append(
                            AntipatternViolation(
...
class AntipatternReporter:
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        for violation in self.violations:
            ...
            summary["by_antipattern"][antipattern_id] = (
                summary["by_antipattern"].get(antipattern_id, 0) + 1
            )
    ...
    def generate_console_report(self) -> str:
        ...
        for file_path, file_violations in sorted(violations_by_file.items()):
            ...
            for violation in sorted(file_violations, key=lambda v: v.line_number):
                ...
                report_lines.append(
                    f"  {severity_icon} Line {violation.line_number}: {violation.name}"
                )
        ...
        sorted_antipatterns = sorted(
            summary["by_antipattern"].items(), key=lambda x: x[1], reverse=True
        )
    ...
    def generate_markdown_report(self) -> str:
        ...
        for file_path, file_violations in sorted(violations_by_file.items()):
            ...
            for violation in sorted(file_violations, key=lambda v: v.line_number):
                ...
                report_lines.append(
                    f"#### {severity_icon} {violation.name} (Line {violation.line_number})"
                )
                report_lines.append("")
                report_lines.append(f"**Message:** {violation.message}")
                report_lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--exclude-patterns", nargs="+", default=[], help="File patterns to exclude"
    )
    ...
    if os.path.isfile(args.path):
        ...
        extensions = [
            f".{pattern.replace('*.', '')}" for pattern in args.include_patterns
        ]
    ...
    if args.severity:
        ...
        violations = [
            v for v in violations if severity_order[v.severity.value] <= min_severity
        ]
