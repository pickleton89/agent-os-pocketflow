...
class TechPatternDetector:
    ...
    def detect_documentation_needs(
        self, spec_text: str, context: str = "create-spec"
    ) -> List[Dict[str, Any]]:
        """
        ...
        for tech, config in self.tech_patterns.items():
            ...
            matches = sum(1 for pattern in config["patterns"] if pattern in spec_lower)
                ...
                suggestions.append(
                    {
                        ...
                        "matched_patterns": [
                            p for p in config["patterns"] if p in spec_lower
                        ],
                        "context": context,
                    }
                )
...
class ProgressiveDisclosure:
    ...
    def get_content_for_level(
        self, documentation_source: str, level: str
    ) -> Dict[str, Any]:
        """
        ...
        cached_content = self._get_cached_content(
            cache_key, level_config["cache_ttl_hours"]
        )
        if cached_content:
...
class VersionManager:
    ...
    def check_compatibility(
        self, tech: str, project_version: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not project_version:
            ...
            report["warnings"].append(
                "Project version unknown - cannot verify compatibility"
            )
        ...
        if not registry_version:
            ...
            report["suggestions"].append(
                "Update documentation registry with version information"
            )
            return report
        ...
        if comparison == "exact_match":
            ...
            report["warnings"].append(
                f"Minor version difference: docs={registry_version}, project={project_version}"
            )
        elif comparison == "major_diff":
            report["status"] = "potentially_incompatible"
            report["warnings"].append(
                f"Major version difference: docs={registry_version}, project={project_version}"
            )
            report["suggestions"].append(
                "Consider updating documentation or project version"
            )
        else:
...
class IntelligentSuggestionSystem:
    ...
    def generate_suggestions(
        ...
        """
        # Detect patterns
        pattern_suggestions = self.pattern_detector.detect_documentation_needs(
            spec_text, context
        )
        ...
        disclosure_level = self._determine_disclosure_level(
            context, workflow_history or []
        )
        ...
        return {
            ...
            "version_compatibility": {
                ...
                "needs_attention": len(
                    [r for r in compatibility_reports if r["warnings"]]
                ),
            },
            "progressive_disclosure": {
                "recommended_level": disclosure_level,
                "sections": self.progressive_disclosure.disclosure_levels[
                    disclosure_level
                ]["sections"],
            },
    ...
    def _determine_disclosure_level(
        self, context: str, workflow_history: List[str]
    ) -> str:
    ...
    def _generate_actionable_recommendations(
        ...
        critical_suggestions = [
            s for s in pattern_suggestions if s["priority"] == "critical"
        ]
            ...
            recommendations.append(
                {
                    ...
                    + ", ".join(s["technology"] for s in critical_suggestions),
        ...
        problematic_versions = [
            r
            for r in compatibility_reports
            ...
            recommendations.append(
                {
                    ...
                    "action": "Review version compatibility for: "
                    + ", ".join(r["technology"] for r in problematic_versions),
        ...
        if context == "execute-tasks":
            implementation_suggestions = [
                s for s in pattern_suggestions if s["priority"] in ["high", "critical"]
            ]
                ...
                recommendations.append(
                    {
                        ...
                        "technologies": [
                            s["technology"] for s in implementation_suggestions
                        ],
                    }
                )
