...
class ValidationFeedbackAnalyzer:
    ...
    def analyze_validation_results(
        ...
        workflow_spec_file: Optional[str] = None,
    ) -> FeedbackLoop:
        ...
        spec_data = (
            self._load_spec_data(workflow_spec_file) if workflow_spec_file else {}
        )

        # Create feedback loop
        feedback_loop = FeedbackLoop(workflow_name=spec_data.get("name", "Unknown"))
        ...
        feedback_loop.auto_fix_actions = [
            ...
            for f in feedback_loop.validation_issues
    ...
    def _parse_text_validation_output(self, content: str) -> List[Dict[str, Any]]:
        ...
        for line in content.split("\n"):
            ...
            if "ERROR" in line.upper():
                issues.append(
                    {"level": "error", "message": line, "category": "general_error"}
                )
            elif "WARNING" in line.upper():
                issues.append(
                    {"level": "warning", "message": line, "category": "general_warning"}
                )
            ...
            elif "IMPORT" in line.upper() and (
                "ERROR" in line.upper() or "FAIL" in line.upper()
            ):
                issues.append(
                    {"level": "error", "message": line, "category": "import_error"}
                )
    ...
    def _create_feedback_from_issue(
        self,
        issue: Dict[str, Any],
        ...
        if category == "missing_implementation" or "TODO" in message:
            return ValidationFeedback(
                ...
                context_needed=self._get_relevant_context_for_todo(
    ...
    def _get_relevant_context_for_todo(
        ...
        for req in context_data.get("requirements", []):
            if any(
                word in req.get("text", "").lower()
                for word in ["database", "api", "process"]
            ):
    ...
    def _identify_context_gaps(
        ...
        if len(context_data.get("requirements", [])) < 3:
            gaps.append(
                "Insufficient requirements - consider adding more detailed specifications"
            )
        ...
        if len(context_data.get("patterns_detected", [])) == 0:
            gaps.append(
                "No clear patterns detected - may need more specific requirements"
            )

        return gaps

    def _generate_iteration_suggestions(
        self, feedback_issues: List[ValidationFeedback], context_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def create_feedback_report(self, feedback_loop: FeedbackLoop, output_file: str):
    ...
    def create_markdown_report(self, feedback_loop: FeedbackLoop, output_file: str):
        ...
        if feedback_loop.suggested_iterations:
            report_lines.extend(["## Iteration Suggestions", ""])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze validation results and create feedback loops"
    )
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="validation_feedback.json",
    ...
    feedback_loop = analyzer.analyze_validation_results(
        args.validation_output, args.context, args.spec
    )
