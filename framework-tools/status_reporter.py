...
class StatusReporter:
    ...
    def start_step(self, step_number: int, description: str):
        ...
        percentage = (
            int((step_number * 100) / self.total_steps) if self.total_steps > 0 else 0
        )
        ...
        print(
            f"📊 Progress: [{progress_bar}] {percentage}% - Step {step_number}/{self.total_steps}: {description}"
        )
    ...
    def complete_operation(self, success: bool = True, final_message: str = None):
        ...
        completion_data = {
            ...
            if success
                ...
                int(self.current_step * 100 / self.total_steps)
    ...
    def print_status_report(self):
        ...
        if summary.get("files_created"):
            ...
            for file_info in summary["files_created"][-5:]:  # Show last 5 files
                size_info = (
                    f" ({file_info['size']} bytes)" if file_info.get("size") else ""
                )
...
class ErrorRecoveryManager:
    ...
    def _recover_template_generation(
        self, error_message: str, context: Dict[str, Any]
    ) -> bool:
    ...
    def _recover_context_extraction(
        self, error_message: str, context: Dict[str, Any]
    ) -> bool:
