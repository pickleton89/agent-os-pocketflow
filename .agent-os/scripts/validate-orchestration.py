...
class OrchestrationValidator:
    ...
    def validate_orchestrator_agent(self):
        ...
        if not agent_path.exists():
            self.add_result("Orchestrator Agent", False, f"Agent file not found: {agent_path}")
            return
        ...
        if missing_sections:
            self.add_result("Orchestrator Agent", False, f"Missing sections: {missing_sections}")
        else:
            self.add_result("Orchestrator Agent", True)
    
    def validate_orchestration_hooks(self):
        """Validate orchestration hooks system."""
        hooks_path = self.agent_os_path / "instructions" / "orchestration" / "orchestrator-hooks.md"
        
        if not hooks_path.exists():
            self.add_result("Orchestration Hooks", False, f"Hooks file not found: {hooks_path}")
            return
        ...
        if missing_hooks:
            self.add_result("Orchestration Hooks", False, f"Missing hooks: {missing_hooks}")
        else:
            self.add_result("Orchestration Hooks", True)
    
    def validate_coordination_config(self):
        """Validate coordination configuration."""
        coord_path = self.agent_os_path / "instructions" / "orchestration" / "coordination.yaml"
        
        if not coord_path.exists():
            self.add_result("Coordination Config", False, f"Coordination file not found: {coord_path}")
            return
        ...
        try:
            ...
            if missing_keys:
                self.add_result("Coordination Config", False, f"Missing keys: {missing_keys}")
                return
            
            # Check coordination map
            coord_map = config["coordination_map"]
            required_instructions = ["plan-product", "create-spec", "execute-tasks"]
            missing_instructions = [instr for instr in required_instructions if instr not in coord_map]
            
            if missing_instructions:
                self.add_result("Coordination Config", False, f"Missing instruction mappings: {missing_instructions}")
            else:
    ...
    def validate_dependency_validation(self):
        ...
        dep_path = self.agent_os_path / "instructions" / "orchestration" / "dependency-validation.md"
        
        if not dep_path.exists():
            self.add_result("Dependency Validation", False, f"Dependency validation file not found: {dep_path}")
            return
        ...
        if missing_elements:
            self.add_result("Dependency Validation", False, f"Missing elements: {missing_elements}")
        else:
    ...
    def validate_instruction_file_integration(self):
        ...
        if all_integrated:
            ...
            self.add_result("Instruction File Integration", False, f"Issues: {missing_integrations}")
    ...
    def validate_template_files(self):
        ...
        if missing_templates:
            self.add_result("Template Files", False, f"Missing templates: {missing_templates}")
        else:
