        ...
        if utils_dir.exists():
            if not any(utils_dir.glob("*.py")):
                self.warnings.append(
                    "Utils directory exists but contains no Python files"
                )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if not any(base in valid_bases for base in base_names):
                    self.warnings.append(
                        f"Node class {node.name} may not extend a valid PocketFlow node type"
                    )
                ...
                for method in required_methods:
                    if method not in all_methods:
                        self.errors.append(
                            f"Node {node.name} missing required method: {method}"
                        )
                ...
                if not exec_methods:
                    self.errors.append(
                        f"Node {node.name} missing exec or exec_async method (found methods: {all_methods})"
                    )
    ...
    def _validate_test_coverage(self):
        ...
        for test_file in test_files:
            ...
            if "class Test" not in content:
                self.warnings.append(
                    f"{test_file.name} may be missing test class structure"
                )
