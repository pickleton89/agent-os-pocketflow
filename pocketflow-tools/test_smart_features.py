...
def test_intelligent_suggestions():
    ...
    print(f"  Pattern suggestions: {result['pattern_suggestions']['total_suggestions']}")
    print(f"  Version issues: {result['version_compatibility']['needs_attention']}")
    print(f"  Disclosure level: {result['progressive_disclosure']['recommended_level']}")
...
def test_main_interface():
    ...
    print(f"  Technologies detected: {len(result['pattern_suggestions']['by_priority'])}")
...
def test_registry_integration():
    ...
    new_suggestions = detector.detect_documentation_needs("We need Django models and views", "create-spec")
    print(f"  Django not in registry - suggestions: {len(new_suggestions)}")
    print(f"  Should be > 0 (needs documentation): {'PASS' if len(new_suggestions) > 0 else 'FAIL'}")
