...
class ContextManager:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.pocketflow_indicators = {
            "RAG": [
                "search", "retrieval", "knowledge", "document", "query", 
                "semantic", "vector", "embedding", "similarity"
            ],
            "AGENT": [
                "agent", "intelligent", "autonomous", "decision", "reasoning",
                "planning", "goal", "task", "conversation"
            ],
            "WORKFLOW": [
                "workflow", "process", "pipeline", "sequence", "orchestration",
                "automation", "batch", "job", "scheduling"
            ],
            "TOOL": [
                "tool", "utility", "function", "operation", "action",
    ...
    def extract_project_context(self, workflow_name: Optional[str] = None) -> ProjectContext:
        ...
        for doc_file in self.docs_dir.glob("**/*.md"):
            ...
            try:
                ...
                context.source_documents.append(str(doc_file.relative_to(self.project_root)))
    ...
    def _extract_requirements(self, content: str, source_file: str, context: ProjectContext):
        ...
        for pattern in list_patterns:
            ...
            for match in matches:
                requirement_text = match.group(1).strip()
                if len(requirement_text) > 15 and any(word in requirement_text.lower() 
    ...
    def _extract_roadmap_info(self, content: str, context: ProjectContext):
        ...
        for match in matches:
            context.complexity_indicators.append(f"Planned phase: {match.group(1).strip()}")
    ...
    def _extract_design_info(self, content: str, context: ProjectContext):
        ...
        common_techs = [
            ...
            'postgresql', 'mongodb', 'redis', 'docker', 'chromadb', 'openai',
            'aws s3', 'google drive', 'vector database', 'rest api'
        ]
    ...
    def _analyze_patterns(self, context: ProjectContext):
        ...
        for pattern_name, indicators in self.pocketflow_indicators.items():
            ...
            if matches >= 2:  # Threshold for pattern detection
                ...
                logger.debug(f"Detected pattern {pattern_name} with {matches} indicators")
    ...
    def create_workflow_specification(self, context: ProjectContext, 
    ...
    def _create_description(self, context: ProjectContext) -> str:
        ...
        functional_reqs = [req for req in context.requirements if req.type == "functional"]
        if functional_reqs:
    ...
    def save_context_analysis(self, context: ProjectContext, 
                             output_file: str = "context_analysis.json"):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract project context from design documents")
    parser.add_argument("--project-root", "-p", default=".", 
                       help="Project root directory")
    parser.add_argument("--workflow-name", "-w", 
                       help="Name of the workflow to generate")
    parser.add_argument("--output", "-o", default="context_analysis.json",
                       help="Output file for context analysis")
    parser.add_argument("--spec", "-s", 
                       help="Output file for workflow specification")
