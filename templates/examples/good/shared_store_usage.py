...
class SharedStoreSchemaNode(Node):
    ...
    def exec(self, prep_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if options["validate_email"]:
            ...
            processed_user["email_domain"] = (
                user_data["email"].split("@")[-1] if "@" in user_data["email"] else None
            )
...
class DataAggregatorNode(Node):
    ...
    def exec(self, prep_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        user_profile = {
            "basic_info": {
                ...
                "name": processed_user.get(
                    "normalized_name", processed_user.get("original_name")
                ),
    ...
    def post(
        ...
        stats["avg_profile_completeness"] = (
            (current_avg * (total_profiles - 1)) + completeness
        ) / total_profiles
...
class SharedStoreValidatorNode(Node):
            ...
            if key in shared:
                if not isinstance(shared[key], expected_type):
                    validation_results[key] = (
                        f"Type mismatch: expected {expected_type.__name__}, got {type(shared[key]).__name__}"
                    )
                else:
    ...
    def exec(self, prep_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        default_values = {}
        if "user_profile" in validation_results and validation_results[
            "user_profile"
        ] in ["missing", "type_mismatch"]:
        ...
        if "processing_metadata" in validation_results and validation_results[
            "processing_metadata"
        ] in ["missing", "type_mismatch"]:
        ...
        if "system_stats" in validation_results and validation_results[
            "system_stats"
        ] in ["missing", "type_mismatch"]:
...
if __name__ == "__main__":
    ...
    shared = SharedStore(
        {
            ...
            "external_data": {"account_type": "premium", "signup_date": "2024-01-15"},
    ...
    print(
        f"  Processed user: {shared.get('processed_user', {}).get('normalized_name')}"
    )
    ...
    print(f"  Action: {action}")
    print(
        f"  Profile completeness: {shared.get('aggregation_summary', {}).get('profile_completeness', 0):.2%}"
    )
